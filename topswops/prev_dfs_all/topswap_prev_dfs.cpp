...
bool search_back(perm p) {
  ...
  const int n = p.size();
  //cout << "  n = " << n << endl;
  const int p_score = do_all_top_swaps_copy(p);
  ...
  while (!stack.empty()) {
    ...
    int score = do_all_top_swaps_copy(*curr_perm);
    ...
    if (score > best_score) {
      ...
      set_current_score(n, best_score, best);
    }
...
void test5() {
  for(int i = 0; i < n_vals_size; ++i) {
    ...
    if (search_back(*p)) {
      set_current_score(n, do_all_top_swaps_copy(*p), *p);
    }
    delete p;
  }
}
...
void test4() {
  ...
  if (search_back(*p37)) {
    set_current_score(37, do_all_top_swaps_copy(*p37), *p37);
  }
}
